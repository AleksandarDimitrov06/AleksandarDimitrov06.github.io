document.addEventListener('DOMContentLoaded', () => {
    ...
    function movePiece(startRow, startCol, endRow, endCol) {
        ...
        // If this is a capture, track the captured piece
        if (isCapture) {
    ...
    // Render the captured pieces in the UI
    function renderCapturedPieces() {
        // Clear the current displays
        whiteCapturesElement.innerHTML = '';
        blackCapturesElement.innerHTML = '';

        // Sort captured pieces by type (pawns first, then knights, bishops, rooks, queen)
        const pieceValue = {
            'pawn': 1,
            'knight': 3,
            'bishop': 3,
        ...
        // Sort the captured pieces by value (ascending)
        const sortPieces = (pieces) => {
            return [...pieces].sort((a, b) => pieceValue[a.type] - pieceValue[b.type]);
        };

        // Get the sorted captured pieces
        const whiteCaptured = sortPieces(capturedPieces.white);
        const blackCaptured = sortPieces(capturedPieces.black);

        // Group pieces by type for better organization in horizontal layout
        ...
        // Get grouped pieces
        const groupedWhiteCaptured = groupPiecesByType(whiteCaptured);
        const groupedBlackCaptured = groupPiecesByType(blackCaptured);

        // Order of piece types to display
        const pieceOrder = ['pawn', 'knight', 'bishop', 'rook', 'queen', 'king'];

        // Render white player's captures (black pieces)
        pieceOrder.forEach(pieceType => {
            const pieces = groupedWhiteCaptured[pieceType] || [];
            pieces.forEach(capturedPiece => {
                const pieceElement = document.createElement('div');
                pieceElement.className = `piece ${capturedPiece.color}-piece`;
                pieceElement.innerHTML = getPieceSymbol(capturedPiece);

                // Force inline styles to prevent inheritance issues
                pieceElement.style.position = 'static';
                pieceElement.style.width = '25px';
                pieceElement.style.height = '25px';
                pieceElement.style.transform = 'none';
                pieceElement.style.margin = '0 3px';
                pieceElement.style.flexShrink = '0';

                whiteCapturesElement.appendChild(pieceElement);
            });
        });

        // Render black player's captures (white pieces)
        pieceOrder.forEach(pieceType => {
            const pieces = groupedBlackCaptured[pieceType] || [];
            pieces.forEach(capturedPiece => {
                const pieceElement = document.createElement('div');
                pieceElement.className = `piece ${capturedPiece.color}-piece`;
                pieceElement.innerHTML = getPieceSymbol(capturedPiece);
                ...
                blackCapturesElement.appendChild(pieceElement);
            });
        });
    }
